...
import eu.semberal.dbstress.actor.ControllerActor.{
  UnitRunError,
  UnitRunFinished,
  UnitRunInitializationFailed,
  UnitRunInitializationFinished
}
import eu.semberal.dbstress.actor.DatabaseActor.{
...
class DatabaseActor(
  ...
  private val dbDispatcher =
    context.system.dispatchers.lookup("akka.dispatchers.db-dispatcher")
  ...
  override def receive: Receive = LoggingReceive {
    case InitConnection =>
      ...
      val future = {
        ...
        urConfig.dbConfig.connectionTimeout
          .map({ timeout =>
            val timeoutFuture = akka.pattern
              .after(timeout.milliseconds, context.system.scheduler) {
                Future.failed(new ConnectionTimeoutException(timeout))
              }(systemDispatcher)
            Future.firstCompletedOf(List(initFuture, timeoutFuture))(
              systemDispatcher
            )
          })
      ...
      future.onComplete {
        case Success(c) =>
          ...
          this.connInitResult =
            Some(DbConnInitResult(Utils.toMillis(start, System.nanoTime())))
        case Failure(e) =>
    ...
    case StartUnitRun =>
      ...
      (1 to urConfig.repeats)
        .foldRight[Future[List[DbCallResult]]](Future.successful(Nil)) {
          (_, prevFuture) =>
            prevFuture.flatMap { l =>
              Future {
                val dbCallId =
                  DbCallId(scenarioId, connectionId, IdGen.genStatementId())
                val start = System.nanoTime()
                connection
                  .map(c =>
                    c.createStatement().autoClosed.apply { statement =>
                      Try {
                        if (
                          statement.execute(
                            urConfig.dbConfig.query
                              .replace(IdGen.IdPlaceholder, dbCallId.toString)
                          )
                        )
                          FetchedRows(
                            Iterator
                              .continually(statement.getResultSet.next())
                              .takeWhile(identity)
                              .length
                          )
                        else
                        ...
                        case Success(result) =>
                          DbCallSuccess(
                            Utils.toMillis(start, System.nanoTime()),
                            dbCallId,
                            result
                          ) :: l
                        case Failure(e) =>
                          logger
                            .warn(s"Query execution failed: ${e.getMessage}")
                          DbCallFailure(
                            Utils.toMillis(start, System.nanoTime()),
                            dbCallId,
                            e
                  ...
                  .getOrElse {
                    val e =
                      new IllegalStateException("Connection not initialized")
                    DbCallFailure(
                      Utils.toMillis(start, System.nanoTime()),
                      dbCallId,
                      e
        ...
        .onComplete {
          case Success(l) =>
            connInitResult match {
              case Some(irr) =>
                s ! UnitRunFinished(unitName, UnitRunResult(irr, l))
              case None =>
                val inner = new IllegalStateException(
                  "Initialization result not available"
                )
